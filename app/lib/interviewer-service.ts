import { scenarios } from "./question-catalog";
import type { InterviewTurnRequest, ProviderId, ProviderMessage } from "./provider-types";
...
export type InterviewerServiceOptions = {
...
function decideOrigin(request: Request, options: InterviewerServiceOptions): OriginDecision {
  ...
  const headerOrigin = forwardedHost ? `${forwardedProtocol}://${forwardedHost}` : requestOrigin;
  const allowed = origin === requestOrigin
    || origin === headerOrigin
    || parseAllowedOrigins(options.allowedOrigins).has(origin);
...
export function handleInterviewerOptions(httpRequest: Request, options: InterviewerServiceOptions = {}) {
  const decision = decideOrigin(httpRequest, options);
  if (!decision.allowed) return json({ error: "Origin is not allowed." }, 403);

  return new Response(null, {
    status: 204,
    headers: {
      "Cache-Control": "no-store",
      ...corsHeaders(decision.origin),
...
export async function handleInterviewerPost(httpRequest: Request, options: InterviewerServiceOptions = {}) {
  const decision = decideOrigin(httpRequest, options);
  if (!decision.allowed) return json({ error: "Origin is not allowed." }, 403);

  try {
    const contentLength = Number(httpRequest.headers.get("content-length") || 0);
    if (contentLength > 250_000) return json({ error: "Provider request is too large." }, 413, decision.origin);
    const request = await httpRequest.json();
    if (JSON.stringify(request).length > 250_000) return json({ error: "Provider request is too large." }, 413, decision.origin);
    if (!validateRequest(request)) return json({ error: "Invalid provider request." }, 400, decision.origin);

    const prompt = buildPrompt(request);
    const providerReply = (await callProvider(request, prompt)).trim();
    if (!providerReply) return json({ error: "The selected provider returned an empty response." }, 502, decision.origin);
